...
class Account():
	...
	def __init__( self, proxies, headers, email, password, progressbar ):
		self.headers = {
	...
	def cloneAccount( self, nEmail, nPassword, createNewAcc ):
		...
		if n_account and o_account:
			...
			for song in songs:
				session.get(
				    'https://www.saavn.com/api.php?_marker=0&entity_type=song&entity_ids={0}&_format=json&__call=library.add'
				    .format( song )
				    )
			...
			for album in albums:
				session.get(
				    'https://www.saavn.com/api.php?_marker=0&entity_type=album&entity_ids={0}&_format=json&__call=library.add'
				    .format( album )
				    )
	...
	def start_download_playlist( self ):
		try:
			playlist = Playlist( self.proxies, self.headers, self.progressbar )
			...
			if playlistIDs is not None:
				...
				for pl in playlistIDs:
					...
					manager.downloadSongs( self.progressbar, songs_json )
			self.logout( session )
		except Exception as e:
			messagebox.showerror( 'Error', e )
		...
		try:
			...
			if albumIDs is not None:
				...
				for albumID in albumIDs:
					try:
						album = Album( self.progressbar, self.proxies, self.headers )
	...
	def start_download_podcast( self ):
		try:
			library_json, session = self.getLibrarySession()
			podcast = Podcast( self.progressbar, self.proxies, self.headers )
