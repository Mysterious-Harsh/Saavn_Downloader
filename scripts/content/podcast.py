...
class Podcast:
	...
	def __init__( self, progressbar, proxies, headers, url=None ):
		...
		for season, season_json in show_json.items():
			...
			for episode in season_json:
				...
				try:
					...
					self.progressbar[ 2 ].set( filename )
					self.progressbar[ 1 ].update()
				except Exception as e:
					print( 'Download Error: {0}'.format( e ) )
				try:
					location = manager.get_download_location( show_name, season_name, filename )
					has_downloaded = manager.start_download(
					    "Show: {} - Season: {} - Episode: {}".format( show_name, season_name, filename ), location,
					    dec_url
					    )
					if has_downloaded:
						# TODO: Add tags for to the podcast episodes
						print( '\n' )
					self.progressbar[ 0 ][ 'value' ] = ( count * 100 ) / n
					self.progressbar[ 1 ].update()
					count += 1
				except Exception as e:
					print( 'Download Error: {0}'.format( e ) )
