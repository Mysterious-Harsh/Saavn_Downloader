...
class Playlist():
	...
	def getPlaylist( self, playlistID=None ):
		...
		response = requests.get(
		    'https://www.jiosaavn.com/api.php?listid={0}&_format=json&__call=playlist.getDetails'.format( playlistID ),
		    verify=False,
	...
	def downloadPlaylist( self ):
		if self.playlistID is not None:
			...
			manager = Manager()
			manager.downloadSongs( self.progressbar, self.getPlaylist() )
