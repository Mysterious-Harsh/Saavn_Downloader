...
class Album():
	...
	def __init__( self, proxies, headers, progressbar, url=None ):
	...
	def getAlbum( self, albumID=None ):
		...
		response = requests.get(
		 'https://www.jiosaavn.com/api.php?_format=json&__call=content.getAlbumDetails&albumid={0}'.
		 format( albumID ),
		 verify=False,
	...
	def downloadAlbum( self, artist_name='' ):
		if self.albumID is not None:
			...
			self.getAlbum()
			if artist_name:
				manager.downloadSongs(  self.progressbar,self.songs_json, self.album_name, artist_name=artist_name )
			else:
				manager.downloadSongs( self.progressbar, self.songs_json, self.album_name )
