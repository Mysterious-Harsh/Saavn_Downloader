...
class Artist():
	...
	def __init__( self, proxies, headers, args, progressbar, url=None ):
	...
	def getArtistID( self, url=None ):
		...
		self.url = "https://www.jiosaavn.com/api.php?__call=webapi.get&token={0}&type=artist&p=&n_song=10&n_album=14&sub_type=&category=&sort_order=&includeMetaTags=0&ctx=web6dot0&api_version=4&_format=json&_marker=0".format(
		    token
		    )
	...
	def getArtistAlbumsIDs( self ):
		...
		print( self.album_IDs_artist, self.artist_name )
	...
	def downloadArtistAllSongs( self ):
		try:
			...
			for n_song_page in range( total_requests ):
				...
				manager = Manager()
				manager.downloadSongs( self.progressbar, songs_json, artist_name=artist_name )
		except Exception as e:
			print( str( e ) )
