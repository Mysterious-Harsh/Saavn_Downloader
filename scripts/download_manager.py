...
class Manager():
	...
	def downloadSongs( self, progressbar, songs_json, album_name='songs', artist_name='Non-Artist' ):
		# print( songs_json )
		progressbar[ 0 ][ 'value' ] = 0
		progressbar[ 2 ].set( "" )
		progressbar[ 1 ].update()
		n = len( songs_json[ 'songs' ] )
		count = 1
		for song in songs_json[ 'songs' ]:
			...
			try:
				...
				messagebox.showerror( 'Error', e )
				print( 'Download Error : {0}'.format( e ) )
			progressbar[ 0 ][ 'value' ] = ( count * 100 ) / n
			# progressbar[ 2 ].set( filename )
			progressbar[ 1 ].update()
			count += 1
		try:
	...
	def addtags( self, filename, json_data, playlist_name ):
		...
		cover = MP4Cover(
		    fd.read(), getattr( MP4Cover, 'FORMAT_PNG' if cover_url.endswith( 'png' ) else 'FORMAT_JPEG' )
		    )
		fd.close()
		audio[ 'covr' ] = [ cover ]
		audio.save()
